...
function generateEnemy(monsterName) {
    const enemies = [
        { name: "史萊姆", img: "assets/img/史萊姆.png", maxHP: 30, currentHP: 30, atk: 5, def: 0, rewardExp: 40, rewardCoin: [50, 100], drops: ["藥水", "鐵劍", "矛", "盾牌", "冰凍術", "再生術", "灼燒術"], lvl: [1, 20] },
        { name: "哥布林", img: "assets/img/哥布林.png", maxHP: 400, currentHP: 60, atk: 30, def: 20, rewardExp: 200, rewardCoin: [200, 300], drops: ["斧刀", "鎖子甲", "中型回血藥水", "中型魔力藥水"], lvl: [10, 40] },
        { name: "骷髏兵", img: "assets/img/骷髏頭.png", maxHP: 1500, currentHP: 200, atk: 100, def: 50, rewardExp: 1000, rewardCoin: [600, 800], drops: ["大寶劍", "厚實的鎧甲", "怒擊", "大型回血藥水", "大型魔力藥水", "石化術"], lvl: [20, 60] },
    ];
    ...
    if (monsterName) {
        // 找到名字對應的怪物
        selectedEnemy = enemies.find(e => e.name === monsterName);
        if (!selectedEnemy) {
            console.warn("找不到怪物名字:", monsterName, "改為隨機生成");
    ...
    selectedEnemy = { ...selectedEnemy }; // 複製一份

    const [minLvl, maxLvl] = selectedEnemy.lvl;
    const lvl = Math.floor(Math.random() * (maxLvl - minLvl + 1)) + minLvl;

    selectedEnemy.maxHP += (lvl - 1) * 4;
    selectedEnemy.currentHP = selectedEnemy.maxHP;
    selectedEnemy.atk += (lvl - 1) * 4;
    selectedEnemy.def += (lvl - 1) * 2;
    selectedEnemy.rewardExp += (lvl - 1) * 4;
    selectedEnemy.lvl = lvl;
...
let lastPlayerPos = { x: 30, y: 60 };
let lastMonsters = [];
let nextSpawnOffset = { x: 110, y: 60 }
let IsAtk = false
let playerSpeed = 3;
function loadMap() {
    const iframe = document.getElementById('gameFrame');
    ...
    <html>
        <body style="margin:0; overflow:hidden;">
            <canvas id="gameCanvas" width="800" height="600"
                style="display:block; margin:0 auto; background:#a8d0a8"></canvas>
        </body>
    </html>`;
}
...
function initMapSystem(iframe) {
    ...
    const keys = {};
    const maxWidth = c.getBoundingClientRect().width;
    const maxHeight = iframe.clientHeight;
    c.tabIndex = 0;
    c.focus();
    doc.addEventListener("keydown", e => keys[e.key.toLowerCase()] = true);
    doc.addEventListener("keyup", e => keys[e.key.toLowerCase()] = false);


    function drawSprite(sprite, startX, startY, pixelSize, palette) {
    ...
    const catSprite = [
        "0110000000000110", "0151100000001100", "0015111111111100", "0115555555555100",
        "0155345553455100", "1555335553355510", "1555555555555510", "1553355555335510",
        "0115555555511100", "0011111111111000", "0000011111100000", "0100115555110000",
        "0155155555551000", "0011555555551000", "0000111111110000", "0000000000000000"
    ];
    const catPalette = { "1": "#b8b8c0", "2": "#ff9eb8", "3": "#303038", "4": "white", "5": "#d4d4db" };

    const goblinSprite = [
        "4440044444440444", "4114411111114114", "0411111111111140", "0041131111311400",
        "0041111111111400", "0004113331114000", "0000411111140000", "0000041111400000",
        "0000411111540000", "0004041155104000", "0000041115500400", "0000041155400000",
        "0000041441140000", "0000411404414000", "0000044000440000", "0000000000000000"
    ];
    const goblinPalette = { "1": "#7ecf5f", "3": "#111111", "4": "#4aa341", "5": "#8b5a2b" };

    const skeletonSprite = [
        "0000444444400000", "0004222222240000", "0042222222224000", "0422322222322400",
        "0422222322222400", "0422222222222400", "0044222222444000", "0000433333400000",
        "0000444444000000", "0000442222440000", "0004041111404000", "0000042222400000",
        "0000041441140000", "0000411404414000", "0000044000440000", "0000000000000000"
    ];
    const skeletonPalette = { "1": "#f2f2f2", "2": "#d6d6d6", "3": "black", "4": "#8a8a8a" };
    ...
    const oakSprite = [
        "0000011111100000", "0000122222220000", "0001222222221000", "0012223332222100",
        "0012233333322100", "0122233333332210", "0122223333222210", "0122222222222210",
        "0012222222222100", "0001222222221000", "0000014444100000", "0000014554100000",
        "0000014554100000", "0000014554100000", "0000014444100000", "0000000000000000"
    ];
    const oakPalette = { "1": "#9fe07a", "2": "#6fba55", "3": "#3f7b2a", "4": "#b07a49", "5": "#875736" };

    const wallSprite = [
        "111100000", "111100000", "111100000", "111100000",
        "111100000", "111100000", "111100000", "111111111",
        "111111111", "111111111"
    ];
    const wallPalette = { "1": "#888888" };


    class obj {
        constructor(sprite, palette, x, y, pixelSize, vx = 0, vy = 0, name = "") {
            this.name = name
            this.sprite = sprite;
            ...
            this.width = sprite[0].length * pixelSize;
            this.height = sprite.length * pixelSize;
            this.vx = vx;
            this.vy = vy;
            this.mask = sprite.map(row => row.split('').map(ch => ch !== "0"));
        }
        draw() { drawSprite(this.sprite, this.x, this.y, this.pixelSize, this.palette); }
    }


    function collidePixel(a, b) {
        const ax1 = a.x, ay1 = a.y, ax2 = a.x + a.width, ay2 = a.y + a.height;
        const bx1 = b.x, by1 = b.y, bx2 = b.x + b.width, by2 = b.y + b.height;
        if (ax2 <= bx1 || ax1 >= bx2 || ay2 <= by1 || ay1 >= by2) return false;
        const startX = Math.max(ax1, bx1), startY = Math.max(ay1, by1);
        const endX = Math.min(ax2, bx2), endY = Math.min(ay2, by2);
        for (let py = startY; py < endY; py++) {
            for (let px = startX; px < endX; px++) {
                const ax = Math.floor((px - a.x) / a.pixelSize);
                const ay = Math.floor((py - a.y) / a.pixelSize);
                const bx = Math.floor((px - b.x) / b.pixelSize);
                const by = Math.floor((py - b.y) / b.pixelSize);
                if (a.mask[ay]?.[ax] && b.mask[by]?.[bx]) return true;
            }
        }
        return false;
    }
    ...
    const cat = new obj(catSprite, catPalette, lastPlayerPos.x, lastPlayerPos.y, 3);
    // const staticWalls = [new obj(wallSprite, wallPalette, 120, 10, 3, 0, 0)];
    // const staticWalls = [new obj(TownSprite, TownPalette, 30, 10, 3, 0, 0)];
    const staticWalls = [];
    const townHouse = new obj(TownSprite, TownPalette, 30, 10, 3, 0, 0, "TownHouse");
    let oak_Y = 30
    let oak_X = 30
    const trees = [
        new obj(oakSprite, oakPalette, 80, 0, 3),
        new obj(oakSprite, oakPalette, 80, 1 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 80, 2 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 80, 3 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 80, 4 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 80, 5 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 80, 9 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 80, 10 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 80, 11 * oak_Y, 3),



        new obj(oakSprite, oakPalette, 200 + 1 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 2 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 3 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 4 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 5 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 6 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 7 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 8 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 9 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 10 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 11 * oak_X, 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 12 * oak_X, 0 * oak_Y, 3),


        new obj(oakSprite, oakPalette, 200 + 1 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 2 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 3 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 4 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 5 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 6 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 7 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 8 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 9 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 10 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 11 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),
        new obj(oakSprite, oakPalette, 200 + 12 * oak_X, maxHeight - 35 + 0 * oak_Y, 3),

    ];
    const monsters = [
        new obj(goblinSprite, goblinPalette, 300, 100, 3, 0, 0, "哥布林"),
        new obj(goblinSprite, goblinPalette, 300, 100, 3, 0, 0, "哥布林"),
        new obj(skeletonSprite, skeletonPalette, 300, 300, 3, 0, 0, "骷髏兵"),
        new obj(skeletonSprite, skeletonPalette, 300, 300, 3, 0, 0, "骷髏兵"),
        new obj(slimeSprite, SlimePalette, 300, 150, 3, 0, 0, "史萊姆"),
        new obj(slimeSprite, SlimePalette, 300, 150, 3, 0, 0, "史萊姆"),
        new obj(slimeSprite, SlimePalette, 300, 150, 3, 0, 0, "史萊姆"),
    ];
    ...
    let frameCounter = 0;
    restorePlayer(cat);
    if (lastMonsters.length == 0) {
        recordPositions(cat, monsters);
    } else {
        respawnMonsters(monsters);
    ...
    function loop() {
        ...
        monsters.forEach(m => {
            ...
            if (dist < aggroRange) {
                ...
                m.vx = Math.cos(angle) * monsterSpeed;
                m.vy = Math.sin(angle) * monsterSpeed;
            } else if (frameCounter % 60 === 0 && Math.random() < 0.5) {

                m.vx = (Math.random() * 2 - 1) * monsterSpeed;
                m.vy = (Math.random() * 2 - 1) * monsterSpeed;
            }
            ...
            if (nextX < 0) { nextX = 0; m.vx *= -1; }
            if (nextX + m.width > maxWidth) { nextX = maxWidth - m.width; m.vx *= -1; }
            if (nextY < 0) { nextY = 0; m.vy *= -1; }
            if (nextY + m.height > maxHeight) { nextY = maxHeight - m.height; }


            let tempM = new obj(m.sprite, m.palette, nextX, nextY, m.pixelSize);
            let blocked = false;
            staticWalls.concat(trees).forEach(o => { if (collidePixel(tempM, o)) blocked = true; });


            let collidePlayer = collidePixel(tempM, cat);
            if (collidePlayer) {
                recordPositions(cat, monsters, m.name)
                m.vx = 0;
                m.vy = 0;
                stopGameLoop(false);
                IsAtk = true
                battleTransition(() => {
                    // stopGameLoop(true)
                    loadBattle(true, generateEnemy(m.name));
                });
            }

            if (!blocked && !collidePlayer) {
                m.x = nextX;
                m.y = nextY;
            }
        ...
        staticWalls.forEach(w => w.draw());
        trees.forEach(t => t.draw());
        townHouse.draw();

        let nextX = cat.x, nextY = cat.y;
        let moving = false;
        if (keys["w"]) { nextY -= playerSpeed; moving = true; }
        if (keys["s"]) { nextY += playerSpeed; moving = true; }
        if (keys["a"]) { nextX -= playerSpeed; moving = true; }
        if (keys["d"]) { nextX += playerSpeed; moving = true; }


        if (nextX < 0) nextX = 0;
        if (nextX + cat.width > maxWidth) nextX = maxWidth - cat.width;
        if (nextY < 0) nextY = 0;
        if (nextY + cat.height > maxHeight) nextY = maxHeight - cat.height;

        let tempCat = new obj(cat.sprite, cat.palette, nextX, nextY, cat.pixelSize);
        let blocked = false;
        staticWalls.concat(trees, monsters).forEach(o => { if (collidePixel(tempCat, o)) blocked = true; });
        if (!blocked) { cat.x = nextX; cat.y = nextY; }
        cat.draw();
        if (collidePixel(tempCat, townHouse)) {
            lastPlayerPos.x = 30
            lastPlayerPos.y = 50
            stopGameLoop();
            parent.loadTown()
    ...
    respawnMonsters(monsters);
    animationId = requestAnimationFrame(loop);
    function stopGameLoop(check = true) {
        if (animationId !== null) {
    ...
    function recordPositions(player, monsters, encounteredMonster = "none") {

        lastPlayerPos.x = player.x;
        lastPlayerPos.y = player.y;
        if (lastMonsters.length == 0) {
            lastMonsters = monsters.map(m => {
                let x, y;
                do {
                    x = nextSpawnOffset.x + Math.random() * (maxWidth - nextSpawnOffset.x - m.width);
                    y = nextSpawnOffset.y + Math.random() * (maxHeight - nextSpawnOffset.y - m.height - nextSpawnOffset.y);                        // Y >= 40 且 <= maxHeight-40
                } while (Math.hypot(x - player.x, y - player.y) < nextSpawnOffset.x);
            ...
            lastMonsters = monsters.map(m => {
                if (m.name === encounteredMonster) {
                    let x, y;
                    do {
                        x = nextSpawnOffset.x + Math.random() * (maxWidth - nextSpawnOffset.x - m.width);
                        y = nextSpawnOffset.y + Math.random() * (maxHeight - nextSpawnOffset.y - m.height - nextSpawnOffset.y);
                    } while (Math.hypot(x - player.x, y - player.y) < nextSpawnOffset.x);
    ...
    function respawnMonsters(monsters) {
        monsters.forEach((m, i) => {
            m.x = lastMonsters[i].x;
            m.y = lastMonsters[i].y;
        });
    }

    function battleTransition(callback) {
        const pixelSize = 16;
        const cols = Math.ceil(maxWidth / pixelSize);
        const rows = Math.ceil(maxHeight / pixelSize);
        let progress = 0;
        let speed = 0.015
        ...
        function animate() {
            ...
            for (let y = 0; y < rows; y++) {
                for (let x = 0; x < cols; x++) {
                    ...
                    if (dist < progress * Math.max(cols, rows) / 2) {
                        if (dist < progress * Math.max(cols, rows) / 5) {
                            ctx.fillStyle = "#ff9999";
                        } else if (dist < progress * Math.max(cols, rows) / 3) {
        ...
        <body style="font-family:sans-serif; text-align:center; background:#f5f5dc; min-height:100vh;">
            <div class="container">
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="col-4">
                        <button class="btn btn-outline-warning w-100 mb-3" onclick="parent.loadMap()">探索</button>
        ...
        <body style="font-family:sans-serif; text-align:center; background:#d1ffd1; min-height:100vh;">
            <div class="container">
                ...
                <div class="fixed-bottom p-3 d-flex justify-content-center border-top" style="background:#d1ffd1">
                    <button class="btn btn-outline-primary me-3" onclick="parent.loadMap()">繼續探索</button>
...
function loadBattle(newEncounter = false, enemy) {
...
<div class="fixed-bottom p-3 d-flex justify-content-center border-top" 
  ...
  <button class="btn btn-outline-success me-3" onclick="parent.loadInventory()">物品欄</button>
  <button class="btn btn-secondary me-3" id="btnRun" onclick="parent.loadMap()">逃跑</button>
</div>
`;
...
function StartAnimation(callback) {
    ...
    function animate() {
        ...
        const grad = ctx.createLinearGradient(0, 0, 0, maxHeight);
        grad.addColorStop(0, "#2b3a2b");
        grad.addColorStop(1, "#5a4b3c");
        ctx.fillStyle = grad;
        ctx.fillRect(0, 0, maxWidth, maxHeight);


        const totalHeight = lines.length * 5 * pixelSize + (lines.length - 1) * pixelSize;
        let startY = maxHeight / 2 - totalHeight / 2;

        lines.forEach((line, lineIndex) => {
            let startX = maxWidth / 2 - (line.length * 5 * pixelSize + (line.length - 1) * pixelSize) / 2;
            for (let i = 0; i < line.length; i++) {
                const ch = line[i];
                const sprite = pixelFont[ch] || pixelFont[" "];
                for (let y = 0; y < sprite.length; y++) {
                    for (let x = 0; x < sprite[y].length; x++) {
                        if (sprite[y][x] === "1") {
                            const dx = startX + x * pixelSize;
                            const dy = startY + y * pixelSize;
                            const dist = Math.hypot(dx - maxWidth / 2, dy - maxHeight / 2);
                            if (dist < progress * Math.max(maxWidth, maxHeight) / 2) {
                ...
                startX += 5 * pixelSize + 1;
            }
            startY += 5 * pixelSize + 1;
        });
...
document.getElementById("btnSpeedUp").addEventListener("click", () => {
    iframe.focus()
    playerSpeed += 1
    speedDisplay.textContent = playerSpeed;
...
document.getElementById("btnSpeedDown").addEventListener("click", () => {
    iframe.focus()
    if (playerSpeed - 1 < 0) {
        notify(`老兄 速度都歸零了 實在沒辦法再減速了`, "#ffd1d1");
        return
    }
    playerSpeed -= 1
    speedDisplay.textContent = playerSpeed;
